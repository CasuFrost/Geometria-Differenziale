...
\begin{figure}[h!]
    ...
    \draw[blue, thick, domain=-1.23:1.23, samples=20] plot (\x*\x, \x*\x*\x);
\end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \draw[->] (0,-4.3) -- (0,2.8) node[above] {$y$};
    \draw[blue, thick, domain=-2.5:2.5, samples=50] plot (\x*\x*\x-4*\x, \x*\x-4);
\end{tikzpicture}
...
\begin{axis}[
    ...
    xmin=-1.5, xmax=1.5,
    ymin=-1.5, ymax=1.5,
    samples=200, % Aumenta il numero di campioni per una curva più liscia
    domain=-pi:2*pi % Intervallo di t per la visualizzazione
]
\addplot[
    domain=-pi/2+0.02:3/2*pi-0.02, % L'intervallo di dominio per la funzione
    samples=200, % Numero di punti per disegnare la curva
    variable=t, % Indica che t è la variabile del parametro
    blue, % Colore della curva
    smooth, % Rende la curva più liscia
    thick % Spessore della curva
] ({sin(deg(2*t))}, {cos(deg(t))}); % Le coordinate (x,y) della curva parametrica
\end{axis}
\end{tikzpicture}
